...
class Rope(object):	
	def __init__(self,mass,constraint_len,*args):
		...
		for point in args:
			v = Vector2.Vector(point[0],point[1])	# create new particle
			a = Vector2.Vector()					# create a force vector for the particle
			
			# append particles to appropriate lists
			self.currentParticlePos.append(v)
			self.oldParticlePos.append(v)
			self.accParticleForces.append(a)
		
		
		self.body_collection = []
	...
	def updateColliders(self):
		...
		for i in range(len(self.currentParticlePos)-1):
			
			collider = sat.Line((self.currentParticlePos[i].x, self.currentParticlePos[i].y),
								(self.currentParticlePos[i+1].x, self.currentParticlePos[i+1].y))
			
			
			collider.type = "dynamic"
			new_property = ShapeProperties()
			new_property.setParent(self)
			new_property.setLineColliders([[collider,(i,i+1)]])
			new_property.setShapeCollider(collider)
			
			self.body_collection.append(new_property)
	...
	def CollideBodies(self):
		...
		for my_property in self.body_collection:
			...
			for body in bodies:
				...
				# do not self collide line segs of the same rope
				if my_property.shape_collider == polyA or my_property.parent == parent:
					continue
					
				
				result = sat.collidePolygon(my_property.shape_collider,polyA)
				...
				if result[0]:
					...
					d = my_property.shape_collider.center - polyA.center
					d_other = polyA.center - my_property.shape_collider
					...
					for line_collider,index in my_property.line_colliders:
					...
					#epsilon = 0.01
					for line,index in my_property.line_colliders:
					...
					my_property.line_colliders = []
					for key in d_keys:
						line = lines_dict[key]
						my_property.line_colliders.append(line)
					...
					for line,index in parent.properties.line_colliders:
						...
						# dist from poly center
						d_vec = true_centroid - my_property.shape_collider.center
						dist = d_vec.get_magnitude()
					...
					for line_collider,index in parent.properties.line_colliders:
						...
						for line,ind in my_property.line_colliders:
	...
	def CollideGeometry(self):
		...
		for my_property in self.body_collection:
			...
			for poly in polygons:
				...
				if my_property.shape_collider == polyA:
					break
	...
	def SatisfyConstraints(self):
		...
		for i in range(1):
			...
			for i in range(len(self.currentParticlePos)-1):
				...
				delta = self.currentParticlePos[i+1]-self.currentParticlePos[i]
				delta2 = delta.dotProduct(delta)
			
				x1= self.currentParticlePos[i]
				x2= self.currentParticlePos[i+1]
				...
				#print x1
				self.currentParticlePos[i] = x1
				self.currentParticlePos[i+1] = x2
...
polyA = sat.DEG22RU([300,150,(250,0,0)],30,100)
...
while True:
	...
	if states[K_LEFT]:
		rope.currentParticlePos[0].x -= 0.8
	if states[K_RIGHT]:
		rope.currentParticlePos[0].x += 0.8
	if states[K_UP]:
		rope.currentParticlePos[0].y -= 0.8
	if states[K_DOWN]:
		rope.currentParticlePos[0].y += 0.8
	...
	#rope.currentParticlePos[0].x = 10
	#rope.currentParticlePos[0].y = 10
